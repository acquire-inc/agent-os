...
- [x] **Phases 61–66: Model intelligence observability surface** — Model Routing operator dashboard, live tenant budget + spend-by-model on Cost, architect picker rationale, per-tier model overrides (endpoint + Settings panel), per-agent MTD spend on Agents. **(closed 2026-06-08)**
- [x] **Phase 67: Month-rollup consolidation** — `currentMonthPrefixUtc()` + `modelRoutingForMonth()` single source of truth; fixed local-vs-UTC drift + silent limit=500 MTD undercount. **(closed 2026-06-08)**
- [x] **V2 P1–P10 (buildable-now portions)** — per-agent episodic memory + reflection (P1/P2), reflexion objectives (P3, mig 0026), prompt self-improvement proposals (P4, mig 0027), anomaly circuit-breaker (P5), critic peer-approval quorum (P6, mig 0028), A2A handoff chains (P7, mig 0030), autonomous manager (P8, mig 0031), control-plane correctness (P9), auto-onboarding Viktor flow (P10). Lifecycle sinks implemented; live wiring operator-gated. **(closed 2026-06-13)**
- [x] **I-001/I-002/I-003 coordination invariants** — tenant-config JSONB validators, offline dispatch/results contract twin, agent lease arbitration (mig 0029). **(closed 2026-06-12)**
- [x] **Phase 68: Publish-readiness review + fixes** — GSD loop; 5 Critical + 6 Warning + 4 Info closed (V2 sink tenant threading, cant-fail apply gate, data-layer scoping, secret echo). Artifacts in `.planning/phases/68-publish-readiness-review/`. **(closed 2026-06-14)**
- [x] **Lead pipeline P3+P4 (build-spec §9)** — mig 0032 (icps/leads/lead_events/suppression_list + RLS), 12 deterministic tool handlers, Discovery + Enrichment+Scoring agent seeds, `pnpm seed:icp-acqu` starter. **(closed 2026-06-19)**
- [x] **Phase 69: Final launch review + fixes** — GSD loop; 3 Critical + 4 Warning + 3 Info closed (apify matrix gate, `tool.lead.update_lead` write-back, icp ownership, transactional insert, normalized phone suppression). Artifacts in `.planning/phases/69-final-launch-review/`. **(closed 2026-06-19)**
- [x] **Phase 70: Review + cleanup + hardening loop** — 4-dimension workflow review with adversarial verification; cleanup pass (dead exports, unused imports, CJS require fix, stale planning state); critical/warning fixes (vault-rotate + access-audit tenant scoping, critic cost routing, apify fail-closed, update_lead hard-disqualifiers). Artifacts in `.planning/phases/70-review-cleanup/`. **(closed 2026-06-26)**
